...
function runTest(name:string, {expected, original = generateA(), update }: { expected: $ReadOnlyArray<Diff>, original?: Lockfile, update: Lockfile }) {
	describe(name, () => {
...
describe('unit/diff-packages.js', () => {
	...
	runTest('diffing http: URL (A -> A4)', { expected: getDiffForA_A4(), update: generateA4() })
})
